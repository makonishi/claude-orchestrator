...
- Apply the delegation authorization procedure below before launch.
...
## Delegation authorization and approval

- Evaluate explicit requests in the current conversation and applicable existing instructions as evidence of authorization. Identify the target code, intended Claude Code research or implementation, and whether sending that code to the external service used by Claude Code is within the authorized scope, including when the code is private. A clear request to delegate the target code can provide this evidence; do not require an additional AGENTS.md entry or repeat confirmation for the same authorized scope.
- A generic request to check that the plugin works does not authorize sending arbitrary private code. Use a minimal synthetic example for such a check. If a necessary transfer is outside or unclear in the existing authorization, ask only about that missing scope before proceeding with it. Permission to research or implement code does not include sending credentials, secret values, or real customer data, nor does it imply permission to commit, publish, or deploy.
- Record the authorization basis and limits in the task brief. In any execution approval request, state the relevant user request or applicable instruction, the destination (Claude Code and its configured external service), the specific paths or data, and the permitted operations and tools. Do not include private code or secret values in the approval explanation itself. For example: “The current request explicitly delegates investigation of <target path> to Claude Code; this execution sends that source to its configured external service for read-only analysis, with no edits or publication.” Use this wording only when those facts are supported.
- Keep normal execution approval controls. This skill does not itself grant permission, and authorization from a different conversation must not be assumed to carry over. Re-evaluate the evidence for a new conversation or any scope expansion; reuse established authorization within the current conversation without redundant questions. Do not promise automatic approval or that execution will never pause.
- If approval review rejects an operation, read the reason. Where the approval process permits, provide missing evidence of existing authorization or choose a narrower permitted alternative. Do not repeat the same rejected action unchanged, bypass the rejection through Claude or another tool, or recommend AGENTS.md edits as a way around it. Continue independent permitted work. If the task remains blocked, explain the rejected operation, that automatic approval review rejected it, and the stated reason; request only the missing authorization or information needed for a permitted next step.
- When validating changes to this procedure, review explicit target-code delegation, a generic functionality check without code-transfer authorization, out-of-scope data transfer, and approval rejection. Also check reuse within the same conversation and re-evaluation in another conversation. Distinguish instruction review from live approval or delegation tests.
